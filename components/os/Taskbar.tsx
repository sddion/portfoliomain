...
export const Taskbar = React.memo(function Taskbar() {
    const { windows, minimizeWindow, focusWindow, logout, toggleSnowfall, showSnowfall, hasMaximizedWindow } = useWindowManager()
    ...
    // Determine if taskbar should be hidden
    const shouldHide = hasMaximizedWindow && !isHovered && !startOpen && !wifiOpen

    return (
        <>
            {/* Hover trigger zone at bottom of screen when taskbar is hidden */}
            {hasMaximizedWindow && (
                <div
                    className="fixed bottom-0 left-0 right-0 h-2 z-[9999]"
                    onMouseEnter={() => setIsHovered(true)}
                />
            )}
            <div
                className={`fixed bottom-0 left-0 right-0 h-10 bg-[var(--os-surface)] border-t border-[var(--os-border)] flex items-center justify-between px-2 z-[9999] backdrop-blur-sm transition-transform duration-200 ${shouldHide ? 'translate-y-full' : 'translate-y-0'}`}
                onMouseEnter={() => setIsHovered(true)}
